...
module.exports.filterListings = async (req, res, next) => {
    ...
    if (!filteredListings.length) {
        ...
        // res.redirect("/listings");
        res.render("listings/noFilterFound.ejs");
...
module.exports.search = async(req, res) => {
    ...
    if(allListings.length !=0 ){
        res.locals.success = `Listings searched by Title : "${element}"`;
        res.render("listings/index.ejs", {allListings});
        return;
    }
    if(allListings.length == 0){
        ...
        if(allListings.length != 0) {
            res.locals.success = `Listings searched by Category : "${element}"`;
            res.render("listings/index.ejs", {allListings});
            return;
        }
    }
    if(allListings.length == 0) {
        ...
        if(allListings.length != 0) {
            res.locals.success = `Listings searched by Country : "${element}"`;
            res.render("listings/index.ejs", {allListings});
            return;
        }
    }
    if(allListings.length == 0) {
        ...
        if(allListings.length != 0) {
            res.locals.success = `Listings searched by Location  : "${element}"`;
            res.render("listings/index.ejs", {allListings});
            return;
        }
    }
